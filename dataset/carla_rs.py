...
class CarlaRS(torch.utils.data.Dataset):
    def __init__(self, cfg):
        ...
        self.video_list = []
        for sub_dir in sorted(os.listdir(self.cfg.root_gt)):
            if not os.path.isdir(os.path.join(self.cfg.root_gt, sub_dir)):
                continue
            for vid in sorted(os.listdir(os.path.join(self.cfg.root_gt, sub_dir))):
                if not os.path.isdir(os.path.join(self.cfg.root_gt, sub_dir, vid)):
                    continue
                self.video_list.append(os.path.join(sub_dir, vid))
        ...
        for video_name in self.video_list:
            ...
            frames_in_video = list(filter(lambda x: True if x.endswith("rs.png") else False, os.listdir(video_path)))
    ...
    def get_frames_name(self, frame_name, num_frames, interval, sampling="n_c"):
        ...
        Args:
            ...
            sampling: the sampling mode.
        """
        frame_idx = frame_name[:4]
        frame_idx_length = len(frame_idx)
        frame_idx = int(frame_idx)

        gt_frame_name_format = "{:0" + str(frame_idx_length) + "d}" + "_gs_" + self.cfg.pos + ".png"
        input_frame_name_format = "{:0" + str(frame_idx_length) + "d}" + "_rs.png"
    ...
    def __getitem__(self, idx):
        if platform.system() == "Windows":
            frame_name = self.frames[idx].split("\\")[-1]
            video_name = os.path.join(*self.frames[idx].split("\\")[:-1])
        else:
            frame_name = self.frames[idx].split("/")[-1]
            video_name = os.path.join(*self.frames[idx].split("/")[:-1])
        ...
        if rs_gs_consistency:
            ...
            gs_suffix = "_gs_" + self.cfg.pos + ".png"
            rs_suffix = "_rs.png"
        ...
        gt_frames = np.stack(
            gt_frames, axis=0).transpose([0, 3, 1, 2])
        input_frames = np.stack(
            input_frames, axis=0).transpose([0, 3, 1, 2])
