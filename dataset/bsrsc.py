...
class BSRSC(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if rs_gs_consistency:
            gt_frames_path = os.path.join(self.cfg.root_gt, video_name, "RS", "{}")
            input_frames_path = os.path.join(self.cfg.root_gt, video_name, "GS", "{}")
        else:
        ...
        if self.cfg.mode == "train" and hasattr(self.cfg, "augmentation"):
            ...
            if self.cfg.get("augmentation_rstsr") and input_frames.shape[1] == 4:
                target_H = input_frames.shape[-2]
                scale = H / target_H
                input_frames[:, -1:] *= scale
